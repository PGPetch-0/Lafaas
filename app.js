...
app.get('/getItemFound',async (req,res)=>{
    const item_id = req.query.item_id;
    const type = req.query.type;
    const sql = `SELECT T.*, GROUP_CONCAT(TC.color SEPARATOR ',') AS color FROM Items_found T, Items_found_color TC WHERE T.item_id = TC.item_id AND T.item_id = ${item_id} GROUP BY item_id`
    const foundQuery = connection.promise().query(sql);
    const [results,field] = await foundQuery;
    if (results[0]) res.json(results[0]);
    else res.json({data: "invalid id"});
})
